...
for(path in paths){
   ...
   console.log(match)
    if (match[1] === variables[variables["bookmark"]]) {
                allOrdners.push(variables["bookmark"]) // "/"
            } else {
...
async function createWidget() {
    ...
    if (isDarkmode){
        variables["backgroundColor"] = "000000",
        variables["timeColor"] = "FFFFFF",
        variables["titlecolor"] = "FFFFFF",
        variables["textColor"] = "FFFFFF"
    } else {}

    console.log(variables["backgroundColor"])
        ...
        if(i+column > variables["entriesCount"]-1){
         ...
         // 
           ...
           let source = sourceTimeBlock.addText(info[i + column][0]+ " • ")
               source.font = Font.mediumSystemFont(variables[widgetSize].sourceThickness);
